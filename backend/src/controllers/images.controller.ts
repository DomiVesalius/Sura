...
export const deleteImage: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (imageToDelete.get('author') != username) return res.status(403).json({ message: `User '${username}' is not authorized to delete this image` });
    ...
    return res.status(200).json({ message: `Image '${imageToDelete.get('title')}' posted by ${username} has been deleted` });
};
